...
export class FakeApiClient implements IApiClient {
  ...
  onUploadFile: (payload: { sessionId: SessionId; name: string; data: string })
  => Promise<RpcResponse<{ name: string; path: string; bytes: number }>> =
    payload => Promise.resolve(ok({ name: payload.name, path: `uploads/${payload.name}`, bytes: 0 }))
  ...
  readonly sessions: IApiClient['sessions'] = {
    ...
    uploadFile: (payload: { sessionId: SessionId; name: string; data: string }) => this.record('session.uploadFile', payload, this.onUploadFile(payload)),
