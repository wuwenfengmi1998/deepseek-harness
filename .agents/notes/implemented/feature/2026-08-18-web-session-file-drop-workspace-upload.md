...
**Host admission.** The handler resolves the live session's `header.cwd`, decodes via the shared canonical-base64 check, refuses empty files, enforces a 32 MiB per-file cap (far below the 160 MiB HTTP carrier body cap), sanitizes the name with `basename` + control-character strip + 120-char truncation (path traversal is impossible; `../../evil.txt` stores as `evil.txt`), and writes with `flag: 'wx'` under `uploads/`, deduplicating collisions by numeric suffix before the final extension (`a.txt` → `a-1.txt`) without a check-then-write race.
